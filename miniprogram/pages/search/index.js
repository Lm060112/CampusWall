...
Page({
  ...
  async search(saveHistory = false) {
    ...
    try {
      ...
      if (!result.result || !result.result.success) {
        throw new Error((result.result && result.result.errMsg) || "search failed");
      }
      this.setData({ results: result.result.data || [] });
    } catch (err) {
      console.warn("cloud search failed, use fallback", err);
      const lower = keyword.toLowerCase();
      const results = FALLBACK_RESULTS.filter((item) => {
        ...
        return typeMatched && (!lower || text.includes(lower));
      });
      this.setData({ results });
    }
    if (saveHistory && keyword) this.saveHistory(keyword);
  },
