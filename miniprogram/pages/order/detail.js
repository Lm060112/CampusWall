Page({
  ...
  loadOrder() {
    ...
    const normalized = {
      ...
      statusStep: this.getStatusStep(order.status),
  ...
  getStatusStep(status) {
    ...
    if (["待取餐", "配送中", "进行中", "待参加"].includes(status)) return 2;
    return 1;
  },
  ...
  getPrimaryAction(order) {
    if (order.status === "已完成") return { text: "再来一单", nextStatus: "" };
    if (order.sourceType === "takeaway") {
      return order.status === "商家备餐"
        ? { text: "模拟开始配送", nextStatus: "配送中", nextText: "骑手正在配送中" }
        : { text: "确认收货", nextStatus: "已完成", nextText: "订单已送达" };
    }
  ...
  updateOrderStatus(status, statusText) {
    ...
    if (updatedOrder) {
      const title = status === "已完成" ? "订单已完成" : status;
      const content = this.getStatusMessage(updatedOrder);
      this.addOrderMessage(updatedOrder, title, content, status);
    }
    this.loadOrder();
  ...
  getStatusMessage(order) {
    if (order.status === "待取餐") return `${order.merchant.name} 已备好，请凭取餐号 ${order.pickupNo} 到店自取`;
    if (order.status === "配送中") return `${order.merchant.name} 的外卖正在配送中，请留意取餐点`;
    if (order.status === "进行中") return `${order.merchant.name} 已被接单，跑腿同学正在处理`;
    if (order.status === "待参加") return `${order.merchant.name} 即将开始，请按时到达`;
